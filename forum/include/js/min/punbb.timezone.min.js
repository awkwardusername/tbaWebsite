...
PUNBB.timezone = (function () {
    ...
    function f() {
        var j = new Date();
        return d(new Date(j.getFullYear(), 0, 1, 0, 0, 0, 0));
    }

    function i() {
        var j = new Date();
        return d(new Date(j.getFullYear(), 5, 1, 0, 0, 0, 0));
    }

    function g() {
        var j = f(), k = i(), l = j - k;
        if (l < 0) {
            return{"utc_offset":j, "dst":1, "hemisphere":a};
        } else {
            if (l > 0) {
                return{"utc_offset":k, "dst":1, "hemisphere":b};
            }
        }
        return{"utc_offset":j, "dst":0, "hemisphere":c};
    }
    ...
    return{determine:function () {
        ...
        j = l.utc_offset + "," + l.dst.toString() + k;
        return{"timezone":h.timezones[j], "key":j};
    }, detect_on_register_form:function () {
        PUNBB.common.addDOMReadyEvent(function () {
            var l = PUNBB.timezone.determine().timezone, k = document.getElementById("register_timezone"), j = document.getElementById("register_dst");
            if (k && j) {
                k.value = l.utc_offset;
                j.value = l.uses_dst ? "1" : "0";
            }
        });
    }};
}());